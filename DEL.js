...
====><=====
=>> Music Cmd <<=
${prefix}play - حط رابط الاغنية من اليوتيوب او حط اسم الاغنية
${prefix}skip - تسوي تخطى للاغنية
${prefix}stop - توقف الاغنية و يخرج البوت من الروم الصوتي
${prefix}vol - تحدد درجة صوت البوت
====><=====
${prefix}np - يوريك وش الاغنية المفتوح الان
${prefix}pause - لتوقيف الاغنية موقتا
${prefix}resume - لاكمال الاغنية
====><=====
=>> Games Cmd <<=
...
quran 1 - القرآن الكريم كامل بصوت الشيخ عبدالباسط عبدالصمد

quran 2 - القرآن الكريم كامل بصوت الشيخ سعد الغامدي

quran 3 -  القرآن الكريم كامل بصوت الشيخ عبد الرحمن السديس وسعود الشريم

quran 4 - القرآن الكريم كامل بصوت الشيخ المعيقلي

quran stop - لـ أيقاف تشغيل البوت 

quran live - لتشغيل القرآن الكريم مباشر

لازم يكون في روم كاتبي باْسم اذكار - اذكار
====><=====
=>> Public Cmd <<=
**`)
}
...
client.on('message', message => {
if (message.author.bot) return;
var channel = message.guild.channels.find("name","اذكار");
if (message.content.startsWith('اذكار')) {
if (!channel) return message.reply("**للأسف لا يوجد روم ** ***اذكار***")

channel.send(`${adkar[Math.floor(Math.random() * adkar.length)]}`)
}
});
